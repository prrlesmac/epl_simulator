...
def continental_league_rules_group_stage():
    return {
        ...
        "knockout_bracket": [
            (1, "Bye"), (16, 17), (8, "Bye"), (9, 24),
            (4, "Bye"), (13, 20), (5, "Bye"), (12, 21),
            (2, "Bye"), (15, 18), (7, "Bye"), (10, 23),
            (3, "Bye"), (14, 19), (6, "Bye"), (11, 22),
        ],
        "knockout_format": {
            "po_r32": "two-legged",
            "po_r16": "two-legged",
...
def continental_league_rules_knockout_stage():
    return {
        ...
        "knockout_draw": [
            ("Liverpool", "Bye"), ("Paris S-G", "Brest"),
            ("Aston Villa", "Bye"), ("Atalanta", "Club Brugge"),
            ("Arsenal", "Bye"), ("PSV Eindhoven", "Juventus"),
            ("Atlético Madrid", "Bye"), ("Real Madrid", "Manchester City"),
            ("Barcelona", "Bye"), ("Benfica", "Monaco"),
            ("Lille", "Bye"), ("Dortmund", "Sporting CP"),
            ("Inter", "Bye"), ("Milan", "Feyenoord"),
            ("Leverkusen", "Bye"), ("Bayern Munich", "Celtic"),
        ],
    }
...
def final_ucl_results():
    return  {
        "eliminated in group stage": [
            "Bologna", "Red Star", "Girona", "RB Leipzig",
            "Slovan Bratislava", "Sparta Prague", "Sturm Graz", "Young Boys",
            "Dinamo Zagreb", "Stuttgart", "Shakhtar", "RB Salzburg",
        ],
        "eliminated in playoff": [
            "Monaco", "Brest", "Juventus", "Celtic",
            "Manchester City", "Sporting CP", "Atalanta","Milan"
        ],
        "eliminated in round of 16": [
            "Feyenoord", "Lille", "Liverpool", "Leverkusen",
            "Club Brugge", "PSV Eindhoven", "Benfica", "Atlético Madrid"
        ],
        "eliminated in quarterfinals": [
            "Aston Villa", "Borussia Dortmund", "Real Madrid", "Bayern Munich"
        ],
...
class TestSimulateLeague:
    ...
    def test_simulate_league_domestic_case_1(self, csv_schedule_data_domestic_case_1, csv_elos_data_domestic_case_1, domestic_league_rules):
        ...
        result = simulate_league(league_rules, mock_schedule, mock_elos, num_simulations=10)

        self.assert_domestic_league_summary(result, mock_schedule)

    def test_simulate_league_domestic_case_2(self, csv_schedule_data_domestic_case_2, csv_elos_data_domestic_case_1, domestic_league_rules):
        """Test simulating a domestic league."""
        # Setup
        league_rules = domestic_league_rules
    ...
    def test_simulate_league_domestic_case_3(self, csv_schedule_data_domestic_case_3, csv_elos_data_domestic_case_1, domestic_league_rules):
        """Test simulating a domestic league."""
        # Setup
        league_rules = domestic_league_rules
        ...
        assert np.isclose(result.loc[result["team"] == "Liverpool", "champion"].values[0], 1.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(top_4)]["top_4"].all(), 1.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(relegation_direct)]["relegation_direct"].all(), 1.0, atol=1e-3)

    def test_simulate_league_continental_case_1(self, csv_schedule_data_continental_case_1, csv_elos_data_continental, continental_league_rules_group_stage):
    ...
    def test_simulate_league_continental_case_2(self, csv_schedule_data_continental_case_2, csv_elos_data_continental, continental_league_rules_group_stage):
        ...
        assert np.isclose(result.loc[result["team"].isin(qualified)]["po_r32"].all(), 1.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(eliminated)]["po_r32"].all(), 0.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(eliminated)]["playoff"].all(), 0.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(eliminated_from_direct_r16)]["direct_to_round_of_16"].all(), 0.0, atol=1e-3)

    def test_simulate_league_continental_case_3(self, csv_schedule_data_continental_case_3_4, csv_elos_data_continental, continental_league_rules_group_stage, final_ucl_results):
        ...
        for rounds in ["po_r32","po_r16","po_r8","po_r4","po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_gs)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32"]:
            assert np.isclose(result.loc[result["team"].isin(qualified_to_ko)][rounds].all(), 1.0, atol=1e-3)
        assert np.isclose(result.loc[result["team"].isin(qualified_to_r16)]["po_r16"].all(), 1.0, atol=1e-3)

    def test_simulate_league_continental_case_4(self, csv_schedule_data_continental_case_3_4, csv_elos_data_continental, continental_league_rules_knockout_stage, final_ucl_results):
    ...
    def test_simulate_league_continental_case_5(self, csv_schedule_data_continental_case_5, csv_elos_data_continental, continental_league_rules_knockout_stage, final_ucl_results):
        ...
        for rounds in ["po_r16","po_r8","po_r4","po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_playoff)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_playoff)][rounds].all(), 1.0, atol=1e-3)
        for rounds in ["po_r32","po_r16"]:
            assert np.isclose(result.loc[result["team"].isin(qualified_to_r16)][rounds].all(), 1.0, atol=1e-3)

    def test_simulate_league_continental_case_6(self, csv_schedule_data_continental_case_6, csv_elos_data_continental, continental_league_rules_knockout_stage, final_ucl_results):
    ...
    def test_simulate_league_continental_case_7(self, csv_schedule_data_continental_case_7, csv_elos_data_continental, continental_league_rules_knockout_stage, final_ucl_results):
        ...
        for rounds in ["po_r8","po_r4","po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_r16)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32","po_r16"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_r16)][rounds].all(), 1.0, atol=1e-3)
        for rounds in ["po_r4","po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_qf)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32","po_r16","po_r8"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_qf)][rounds].all(), 1.0, atol=1e-3)
        for rounds in ["po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_sf)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32","po_r16","po_r8","po_r4"]:
            assert np.isclose(result.loc[result["team"].isin(eliminated_in_sf)][rounds].all(), 1.0, atol=1e-3)
        for rounds in ["po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(runner_up)][rounds].all(), 0.0, atol=1e-3)
        for rounds in ["po_r32","po_r16","po_r8","po_r4","po_r2"]:
            assert np.isclose(result.loc[result["team"].isin(runner_up)][rounds].all(), 1.0, atol=1e-3)
        for rounds in ["po_r32","po_r16","po_r8","po_r4","po_r2","po_champion"]:
            assert np.isclose(result.loc[result["team"].isin(champion)][rounds].all(), 1.0, atol=1e-3)
