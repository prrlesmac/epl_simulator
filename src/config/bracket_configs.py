...
RAW_TABLE = """1 E F G H I J K L 3E 3J 3I 3F 3H 3G 3L 3K
...
MATCH_KEYS = ['A1','B1','D1','E1','G1','I1','K1','L1']


def parse_table(raw_text):
    """
    Parse the raw FIFA table into structured rules.
    Each line: <row_num> <8 group letters> <8 target slots>
    """
    rules = []

    for line in raw_text.splitlines():
        parts = line.strip().split()
        if len(parts) == 17:  # row_num + 8 groups + 8 targets
            groups = parts[1:9]
            targets = parts[9:17]
            rules.append((groups, targets))
...
def build_mapping(rules):
    result = {}
    for groups, targets in rules:
        key = tuple(sorted(groups))
        # Invert: {'3E': 'A1', ...} -> {letter+digit: match_slot}
        slot_map = {
            f"{t[1]}{t[0]}": slot   # '3E' -> 'E3'
            for slot, t in zip(MATCH_KEYS, targets)
        }
        result[key] = slot_map
    return result
...
def get_mapping():
    rules = parse_table(RAW_TABLE)
    mapping = build_mapping(rules)

    if len(mapping) != 495:
        raise ValueError(f"Expected 495 mappings, got {len(mapping)}")

    return mapping


fifa_wc_third_place_mapping = get_mapping()
